...
const STAGES = [
  // LOCAL PATCH（my-router-standard）：把本机真实存在的 ast_grep 与 pi-blackhole 的
  // recall 挂进阶段 0（只读搜索 / 记忆检索）。engram_* 是本机没有的上游插件工具名，
  // restrict 的 GLOBAL_SAFE ∩ restrictableNames 会照旧剔除它们。
  { name: '了解/对齐', tools: ['read', 'glob', 'grep', 'web_search', 'ask_user_question', 'ast_grep', 'recall', 'engram_recall', 'engram_verify', 'engram_respond'] },
